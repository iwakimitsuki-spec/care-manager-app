...
- 話者が交替する（つまり別の人が話し始める）ごとに、必ず「1行の空白行（空行）」を挟んで段落を分け、「話者名: 発言内容」の形式で出力すること。
...
添付された音声データは、ケアマネージャーと利用者（または家族等）のリアルな会話を録音したものです。

【絶対に守るべき重大な禁止事項】
・ハルシネーション（情報のねつ造・捏造）を固く禁じます。
...
【出力ルール】（※聞こえた場合のみ以下を作成）
■ 話者分離スクリプト
（実際の音声からのみ誰が発言したかを分類し、「ケアマネ:」「利用者:」などと台本化してください。相槌の整理は構いませんが、内容は絶対に変えないこと。）
（※注意：話者が交替する、つまり別の人が話し始めるごとや、話の間に間がある場合は、必ず「1行の空白行（空行）」を挟んで見やすく出力してください）

■ 要点メモ
（上記の「実際に録音されていた会話」の中でのみ、特筆すべき重要ポイントや本人の意向を箇条書きで抽出してください）`;
